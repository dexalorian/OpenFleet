...
onMounted(() => {
  ...
  map.on('contextmenu', (e) => { ptrLineCoords.visible = true; selectedGeoPnt.lat = e.latlng.lat; selectedGeoPnt.lng = e.latlng.lng; } )
...
map.on('moveend', e => localStorage.setItem('mapframecoords', map.getCenter().lat+'+'+map.getCenter().lng))
  ...
  map.on('resize', (e) =>  {line = map.latLngToContainerPoint( { lat: selectedGeoPnt.lat, 
    ...
    watch( () => myPhotos.photos , () => {console.log('redraw triggered'); clusters.clearLayers(); 
